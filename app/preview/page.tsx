...
function Pv() {
  ...
  const dl=()=>{if(!result||!m)return;const b=new Blob([result],{type:'text/plain;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=m.name+'.'+(fmt==='pdf'?'txt':fmt);a.click()}
  ...
  return(
    <div className="max-w-[760px] mx-auto pt-6 pb-16 animate-in">
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex gap-1.5"><select value={fmt} onChange={e=>setFmt(e.target.value)} className="px-2.5 py-1.5 border border-white/10 rounded-[5px] text-[12px] bg-[#141417] text-white">{(m.output_formats||['pdf']).map((f:string)=><option key={f}>{f.toUpperCase()}</option>)}</select><button onClick={dl} className="px-3 py-1.5 bg-[#00D4AA] text-[#09090B] font-semibold text-[12px] rounded-md">다운로드</button><button onClick={()=>router.push('/execute?id='+m.id)} className="px-3 py-1.5 border border-white/10 text-[#A1A1AA] text-[12px] rounded-md">재실행</button></div>
      </div>
      <div className="border border-white/10 rounded-[10px] overflow-hidden">
        <div className="flex items-center gap-2 px-3.5 py-2 bg-[#1C1C20] border-b border-white/[.06]"><div className="flex gap-1"><span className="w-2 h-2 rounded-full bg-[#FF5F57]"/><span className="w-2 h-2 rounded-full bg-[#FEBC2E]"/><span className="w-2 h-2 rounded-full bg-[#28C840]"/></div><div className="flex-1 bg-[#111114] border border-white/[.06] rounded px-2.5 py-1 text-[11px] text-[#63636E] font-mono">{m.id.toLowerCase()}-output.{fmt}</div></div>
