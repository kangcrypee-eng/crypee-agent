  ...
  return(
    <div className="pt-6 pb-16 animate-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">{list.map(m=>
        <div key={m.id} onClick={()=>m.mode==='alert'?router.push('/alerts/setup?module='+m.id):setSelected(m)} className="rounded-[10px] p-4 cursor-pointer border transition-all hover:opacity-90 relative" style={{background:'var(--surface)',borderColor:'var(--border)'}}>
      ...
      {selected&&<div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={()=>{setSelected(null);setShowSample(false)}}>
        ...
        <div className="relative rounded-xl w-full max-w-[560px] max-h-[85vh] overflow-y-auto border" style={{background:'var(--surface)',borderColor:'var(--border)'}} onClick={e=>e.stopPropagation()}>
          <div className="p-6">
            ...
            {/* 예시 보기 / 접기 */}
            {selected.sample_output&&!showSample&&<button onClick={()=>setShowSample(true)} className="w-full mb-4 py-2.5 rounded-lg text-[13px] border transition-all hover:opacity-80" style={{borderColor:'var(--accent-border)',color:'var(--accent)'}}>예시 결과물 보기 (무료)</button>}

            {showSample&&selected.sample_output&&<div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[12px] font-semibold" style={{color:'var(--text-secondary)'}}>예시 결과물</span>
              ...
              <div className="rounded-lg overflow-hidden border" style={{borderColor:'var(--border-strong)'}}>
                <div className="p-5 max-h-[300px] overflow-y-auto text-[12.5px] leading-[1.7]" style={{background:'var(--preview-bg)',color:'var(--preview-text)'}} dangerouslySetInnerHTML={{__html:'<p>'+render(selected.sample_output)+'</p>'}}/>
              </div>
            </div>}
