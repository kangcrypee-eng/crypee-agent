  ...
  return(
    <div className="pt-6 pb-16 animate-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">{list.map(m=>
        <div key={m.id} onClick={()=>router.push('/execute?id='+m.id)} className="bg-[#141417] border border-white/[.06] rounded-[10px] p-4 cursor-pointer hover:border-white/10 hover:bg-[#1C1C20] transition-all relative">
          ...
          <div className="flex items-center gap-1.5 mt-2.5 text-[11px] text-[#63636E]"><span>◆ {m.credit_cost}</span><span className="w-0.5 h-0.5 rounded-full bg-[#63636E]"/><span>{(m.uses||0).toLocaleString()}회</span></div>
        </div>
      )}</div>}
    </div>
  )
