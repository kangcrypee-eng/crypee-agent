...
export async function GET(request: NextRequest) {
  ...
  const generateAI = async () => {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: mod.ai_model || 'claude-sonnet-4-6', max_tokens: mod.max_tokens || 16384, temperature: mod.temperature ?? 0.3, system: mod.system_prompt || '', messages: [{ role: 'user', content: fullPrompt }] }),
    })
    const data = await res.json()
