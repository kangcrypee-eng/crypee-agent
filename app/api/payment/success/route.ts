...
export async function GET(request: NextRequest) {
  ...
  const generateAI = async () => {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: mod.id?.startsWith('BP') ? 'claude-sonnet-4-6' : (mod.ai_model || 'claude-sonnet-4-6'), max_tokens: mod.id?.startsWith('BP') ? 16384 : (mod.max_tokens || 4096), temperature: mod.temperature ?? 0.3, system: (mod.system_prompt || '') + (mod.id?.startsWith('BP') ? '\n\n[중요 지시사항]\n- 반드시 모든 섹션을 빠짐없이 끝까지 완성하세요\n- 절대 중간에 생략하거나 요약하지 마세요\n- 각 항목을 구체적이고 상세하게 작성하세요\n- 마지막 섹션까지 동일한 품질과 분량을 유지하세요' : ''), messages: [{ role: 'user', content: fullPrompt }] }),
    })
    const data = await res.json()
