...
function Exec() {
  ...
  useEffect(()=>{if(id)supabase.from('modules').select('*').eq('id',id).single().then(({data})=>{if(data){setM(data);setMode(data.mode);if(data.mode==='chat')initC(data)};setLd(false)})},[id])
  ...
  return(
    <div className="max-w-[680px] mx-auto pt-6 pb-16 animate-in">
      ...
      :mode==='form'?<div className="bg-[#141417] border border-white/[.06] rounded-[10px] p-5">
        {fields.length>0?fields.map((f:any)=><div key={f.key} className="mb-3.5"><label className="block text-[11.5px] font-medium text-[#A1A1AA] mb-1.5">{f.label}{f.required&&<span className="text-[#EF5B5B]"> *</span>}</label>
        ...
        </div>):<div className="mb-3.5"><label className="block text-[11.5px] font-medium text-[#A1A1AA] mb-1.5">추가 정보</label><textarea value={fd.extra||''} onChange={e=>setFd({...fd,extra:e.target.value})} placeholder="추가 정보 입력 (선택)" className="inp min-h-[80px]"/></div>}
